...
@Repository
public interface Report2Repository extends JpaRepository<ReportMontoVentaDTO, Long> {
	@Query(value = "select row_number() over(order by name_store) as id , x.* from ("
			+ "select sum(od.quantity) quantity, s.name name_store, p.name name_product "
